...
export const getLotteryBetHistory = async (req, res) => {
    try {
        ...
        if (dataType === 'live') {
            ...
            if (req.query.startDate && req.query.endDate) {
                ...
                if (endDate > maxAllowedDate) {
                    return apiResponseErr([], false, statusCode.badRequest, 'The date range for backup data should not exceed 3 months.',res);
                }
            } else {
                const today = new Date();
        ...
        const { rows: purchaseLotteries, count: totalRecords } = await PurchaseLottery.findAndCountAll({
